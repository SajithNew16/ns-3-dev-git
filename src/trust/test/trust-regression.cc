 ...
 * \ingroup tests
 *
 * \brief TRUST regression test suite
 */
class TrustRegressionTestSuite : public TestSuite
{
public:
  TrustRegressionTestSuite () : TestSuite ("routing-trust-regression", SYSTEM)
  {
    SetDataDir (NS_TEST_SOURCEDIR);
    // General RREQ-RREP-RRER test case
    AddTestCase (new ChainRegressionTest ("trust-chain-regression-test"), TestCase::QUICK);
...
} g_trustRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest (const char * const prefix, Time t, uint32_t size, Time arpAliveTimeout)
  : TestCase ("TRUST chain regression test"),
    m_nodes (0),
    m_prefix (prefix),
    m_time (t),
    m_size (size),
...
ChainRegressionTest::CreateDevices ()
{
  ...
  // 2. Setup TCP/IP & TRUST
  TrustHelper trust; // Use default parameters here
  InternetStackHelper internetStack;
  internetStack.SetRoutingHelper (trust);
  ...
  streamsUsed += trust.AssignStreams (*m_nodes, streamsUsed);
  // TRUST uses m_size more streams
