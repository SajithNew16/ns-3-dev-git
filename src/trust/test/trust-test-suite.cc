 ...
 */
#include "ns3/test.h"
#include "ns3/trust-neighbor.h"
#include "ns3/trust-packet.h"
#include "ns3/trust-rqueue.h"
#include "ns3/trust-rtable.h"
...
{
  TypeHeaderTest () : TestCase ("TRUST TypeHeader")
  {
  }
  virtual void DoRun ()
  {
    TypeHeader h (TRUSTTYPE_RREQ);
    NS_TEST_EXPECT_MSG_EQ (h.IsValid (), true, "Default header is valid");
    NS_TEST_EXPECT_MSG_EQ (h.Get (), TRUSTTYPE_RREQ, "Default header is RREQ");

    Ptr<Packet> p = Create<Packet> ();
    p->AddHeader (h);
    TypeHeader h2 (TRUSTTYPE_RREP);
...
{
  RreqHeaderTest () : TestCase ("TRUST RREQ")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepHeaderTest () : TestCase ("TRUST RREP")
  {
  }
  virtual void DoRun ()
  {
...
{
  RrepAckHeaderTest () : TestCase ("TRUST RREP-ACK")
  {
  }
  virtual void DoRun ()
  {
...
{
  RerrHeaderTest () : TestCase ("TRUST RERR")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for TRUST routing table entry
 */
...
struct TrustRqueueTest : public TestCase
{
  TrustRqueueTest () : TestCase ("Rqueue"),
                      q (64, Seconds (30))
  {
  }
  virtual void DoRun ();
...
TrustRqueueTest::DoRun ()
{
  ...
  h.SetDestination (Ipv4Address ("1.2.3.4"));
  h.SetSource (Ipv4Address ("4.3.2.1"));
  Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback (&TrustRqueueTest::Unicast, this);
  Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback (&TrustRqueueTest::Error, this);
  ...
  Simulator::Schedule (q.GetQueueTimeout () + Seconds (1), &TrustRqueueTest::CheckTimeout, this);
...
void
TrustRqueueTest::CheckSizeLimit ()
{
 ...
 * \brief Unit test for TRUST routing table entry
 */
struct TrustRtableEntryTest : public TestCase
{
  TrustRtableEntryTest () : TestCase ("RtableEntry")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief Unit test for TRUST routing table
 */
struct TrustRtableTest : public TestCase
{
  TrustRtableTest () : TestCase ("Rtable")
  {
  }
  virtual void DoRun ()
  {
 ...
 * \ingroup tests
 *
 * \brief TRUST test suite
 */
class TrustTestSuite : public TestSuite
{
public:
  TrustTestSuite () : TestSuite ("routing-trust", UNIT)
  {
    ...
    AddTestCase (new TrustRqueueTest, TestCase::QUICK);
    AddTestCase (new TrustRtableEntryTest, TestCase::QUICK);
    AddTestCase (new TrustRtableTest, TestCase::QUICK);
  }
} g_trustTestSuite; ///< the test suite
