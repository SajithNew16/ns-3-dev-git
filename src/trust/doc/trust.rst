...
Ad Hoc On-Demand Distance Vector (TRUST)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (TRUST) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 TRUST model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the TRUST-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the TRUST model lives in the directory `src/trust`.

Design
++++++

Class ``ns3::trust::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::trust::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::trust::RouteInput``,
...
this functionality are attributes of ``ns3::trust::RoutingProtocol``. 
...
TRUST discovers routes on demand.  Therefore, the TRUST model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in trust-rqueue.cc. A smart pointer to 
...
* This TRUST implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from TRUST-UU implementation and can be disabled.
...
the assertion from the TRUST RFC that TRUST works over UDP.  This model uses
